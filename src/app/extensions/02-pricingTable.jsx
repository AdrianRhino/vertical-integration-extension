...
import { Table, TableHead, TableBody, TableRow, TableHeader, TableCell, Box, Heading, Button, Text, Panel, PanelBody, Flex, StepperInput, Input, NumberInput } from "@hubspot/ui-extensions";
...
const PricingTable = ({ order, setOrder, runServerlessFunction, actions, setCanGoNext }) => {
  ...
  const searchProducts = async () => {
    const res = await runServerlessFunction({ 
        name: 'viProxy', 
        parameters: { action: 'searchProducts', supplierKey: order.supplier } 
    });
    if (res.response.ok) {
  ...
  return (
    <Box direction="column" gap="medium">
      ...
      <Flex gap="small">
         <Button onClick={() => setShowProductPanel(!showProductPanel)} variant="secondary" size="sm">
            {showProductPanel ? "Hide Catalog" : "Add From Catalog"}
         </Button>
      </Flex>

      {showProductPanel && (
         <Panel title={`Catalog (${order.supplier})`}>
             <PanelBody>
                 <Button onClick={searchProducts} size="xs" variant="tertiary">Load Top Products</Button>
                 <Table>
                     <TableHead>
                         <TableRow>
